...
import OrderDB, { testConnection } from './mongo_orders_db.js';
...
window.addEventListener('beforeunload', function() {
    ...
    if (storedOrder) {
        try {
            // すでに保存されているデータを使用
            localStorage.setItem('currentOrder_backup', storedOrder);
            console.log('unload前にバックアップ作成:', storedOrder);
        } catch (e) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function restoreCurrentOrder() {
        ...
        let backupOrder = localStorage.getItem('currentOrder_backup');
        ...
        if (savedOrder) {
            try {
                console.log('  通常ストレージからデータを復元します...');
                ...
                // 注文状態をUIに反映
                updateOrderStatus();
                console.log('✓ 注文状態UI更新完了 - 通常ストレージから復元');
                ...
                console.error('✗ 通常ストレージからの復元に失敗:', error);
        ...
        if (backupOrder) {
            try {
                ...
                currentOrder = JSON.parse(backupOrder);
                globalCurrentOrder = {...currentOrder}; // グローバル変数にも反映
                // バックアップから復元したデータを通常のストレージにも保存
                localStorage.setItem('currentOrder', backupOrder);
                // 注文状態をUIに反映
                updateOrderStatus();
    ...
    productButtons.forEach(button => {
        button.addEventListener('click', async () => {
            ...
            if (product) {
                ...
                // ローカルストレージに保存 - 確実に保存するため複数回行う
                try {
                    saveCurrentOrder();
                    // バックアップも作成
                    localStorage.setItem('currentOrder_backup', JSON.stringify(currentOrder));
                    ...
                    localStorage.setItem('currentOrder_check', JSON.stringify(saveCheck));
                } catch (e) {
    ...
    setTimeout(() => {
        ...
        // localStorageの全キーを確認
        console.log('  LocalStorageの全キー:');
        for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);
            const value = localStorage.getItem(key);
            const preview = value ? `${value.length}文字` : '空';
